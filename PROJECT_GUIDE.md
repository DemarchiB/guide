...
Este é o ponto de entrada de um conjunto de convenções montado em `docs/guide/` de cada projeto. **Leia este arquivo no início de toda tarefa**; ele é curto de propósito e diz o que mais ler. O resto do conjunto é lido sob demanda — ler tudo nunca é necessário. A declaração de um domínio no projeto é um catálogo de práticas disponíveis, não uma ordem para carregar todos os arquivos.
...
6. **Documento novo parte do seu template**, quando houver; o projeto não guarda cópias de templates. (`estrutura.md`)
...
| criar, usar ou manter uma Skill | `practices/ia-ferramentas.md` e `templates/skill.md` |
...
| executar ou delegar uma tarefa como agente; revisar código gerado | `practices/ia.md` |
| configurar ferramenta, subagente ou adaptador de IA | `practices/ia-ferramentas.md` |
| tarefa coberta por um domínio que o projeto declarou aplicável | o arquivo do domínio, somente se o "Leia quando" da tabela abaixo ocorrer |
| **alterar este conjunto** | `manutencao-do-conjunto.md` e `AGENTS.md` do conjunto |

## Domínios de prática

Cada domínio é um arquivo em `practices/`. Um projeto declara no seu `AGENTS.md` quais se aplicam; essa declaração não torna os demais arquivos leitura obrigatória.
...
| `practices/ia.md` | todo projeto com agente de IA | executar ou delegar tarefa, revisar código gerado ou decidir autorização |
...
Guias de apoio não são carregados por serem declarados no projeto; leia-os somente quando o workflow indicado ocorrer.

| Guia | Leia quando for... |
| --- | --- |
| `practices/ia-ferramentas.md` | criar, usar ou manter Skill, subagente ou adaptador; configurar ferramenta de IA |
